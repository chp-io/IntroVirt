...
static std::string read_file(const boost::filesystem::path& p) {
...
static bool is_domain_id_valid(uint32_t domain_id) {
    boost::filesystem::path proc("/proc");
    boost::filesystem::path commPath = proc / std::to_string(domain_id) / "comm";
...
static std::string get_domain_name(uint32_t domain_id) {
    ...
    boost::filesystem::path proc("/proc");
    boost::filesystem::path cmdLinePath = proc / std::to_string(domain_id) / "cmdline";
